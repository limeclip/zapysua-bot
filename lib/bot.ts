...
async function sendClientWelcome(ctx: BotContext, master: Master): Promise<void> {
  ...
  const keyboard = new InlineKeyboard().webApp("📅 Записатися на прийом", clientUrl);
  const caption = `👋 Привіт!\nЦе AI-адміністратор *${master.business_name}*\n\nОберіть послугу та запишіться на зручний час.`;
  const plainText = `👋 Привіт!\nЦе AI-адміністратор ${master.business_name}\n\nНатисніть кнопку нижче, щоб записатися:`;
  if (master.logo_url) {
    await ctx.replyWithPhoto(master.logo_url, { caption, parse_mode: "Markdown", reply_markup: keyboard });
  } else {
...
async function sendMasterPanel(ctx: BotContext, master: Master): Promise<void> {
  ...
  await ctx.reply(
    `Раді вас бачити, ${master.business_name}! 👋\n\nВідкрийте ваш кабінет, щоб керувати запитами.`,
...
async function sendGuestHelp(ctx: BotContext): Promise<void> {
  await ctx.reply(
    "👋 Ласкаво просимо до ZapysUa!\n\n" +
      ...
      "1. Перейдіть за посиланням від майстра (t.me/…?start=slug або ?startapp=slug)\n" +
      "2. Якщо бот не відповів — надішліть у чат: /start slug_майстра\n" +
      "3. Натисніть кнопку «Записатися на прийом»\n\n" +
      "Майстер? Натисніть кнопку нижче:",
    { reply_markup: new InlineKeyboard().url("💼 Зареєструватися як майстер", getWebAppBaseUrl()) },
  );
}

export const bot = new Bot<BotContext>(getBotToken());

// Команда /start реєструється ПЕРШОЮ, до middleware
bot.command("start", async (ctx) => {
  ...
  if (payload && payload.toLowerCase() !== "start") {
    try {
      ...
      console.log("[bot /start] findMasterByStartParam:", referredMaster ? referredMaster.id : "не знайдено");
      if (referredMaster) {
  ...
  // Якщо payload немає – перевіряємо, чи це майстер
...
// Middleware – отримуємо майстра (без автоматичного створення)
